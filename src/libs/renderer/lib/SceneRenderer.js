...
export class SceneRenderer {
  ...
  renderTextLayer(targetCanvas, layer, keyframe) {
    ...
    context.fillStyle =
      'rgba(' + keyframe.color.red + ',' + keyframe.color.green + ',' + keyframe.color.blue + ',1)';
    context.globalAlpha = keyframe.image_fade_alpha;
  ...
  renderTargetScene(targetCanvas, targetScene, playTime) {
    ...
    for (var i = 0; i < this.targetLayers.length; i++) {
      ...
      if (keyframes.length === 1 || playTime <= 0) {
        //1개의 키프레임만 있을때, 해당 키프레임으로 렌더링한다.
      ...
      } //if layer animation is repeatable
      else {
        ...
        for (var j = 0; j < keyframes.length - 1; j++) {
          if (
            keyframes.at(j).timeLabel <= manipulatedPlaytime &&
            manipulatedPlaytime < keyframes.at(j + 1).timeLabel
          ) {
            let interpolatedKeyframe = null;

            if (targetLayer.layerType === 'image') {
              interpolatedKeyframe = Keyframe.interpolate(
                keyframes[j],
                keyframes[j + 1],
                manipulatedPlaytime,
              );
            } else if (targetLayer.layerType === 'text') {
              interpolatedKeyframe = TextKeyframe.interpolate(
                keyframes[j],
                keyframes[j + 1],
                manipulatedPlaytime,
              );
