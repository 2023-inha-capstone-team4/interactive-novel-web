...
export class Keyframe {
  ...
  static interpolate(k1, k2, time) {
    ...
    var newPosition = new Vector2D(
      (1.0 - ratio) * k1.position.x + ratio * k2.position.x,
      (1.0 - ratio) * k1.position.y + ratio * k2.position.y,
    );
    var newScale = new Vector2D(
      (1.0 - ratio) * k1.scale.x + ratio * k2.scale.x,
...
export class TextKeyframe extends Keyframe {
  ...
  static interpolate(k1, k2, time) {
    ...
    return new TextKeyframe(
      ncKeyframe.timeLabel,
      ncKeyframe.position,
      ncKeyframe.scale,
      ncKeyframe.rotation,
      ncKeyframe.image_fade_alpha,
      newColor,
    );
  }
}
