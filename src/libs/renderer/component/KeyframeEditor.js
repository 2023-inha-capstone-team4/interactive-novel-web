...
function KeyframeEditor() {
  ...
  const playButton = new CanvasButton(
    new Vector2D(300, 10),
    20,
    20,
    'https://static.vecteezy.com/system/resources/previews/010/151/781/non_2x/play-button-icon-sign-design-free-png.png',
    onClickPlay,
  );
  const pauseButton = new CanvasButton(
    new Vector2D(300 + 90, 10),
    20,
    20,
    'https://upload.wikimedia.org/wikipedia/commons/thumb/7/71/Eo_circle_red_pause.svg/2048px-Eo_circle_red_pause.svg.png',
    onClickPause,
  );
  const stopButton = new CanvasButton(
    new Vector2D(300 + 180, 10),
    20,
    20,
    'https://cdn-icons-png.flaticon.com/512/4029/4029011.png',
    onClickStop,
  );
  const addKeyframeButton = new CanvasButton(
    new Vector2D(40, 10),
    20,
    20,
    'https://pngimg.com/d/plus_PNG109.png',
    onClickAddKeyframe,
  );
  const EditKeyframeButton = new CanvasButton(
    new Vector2D(100, 10),
    20,
    20,
    'https://iconsplace.com/wp-content/uploads/_icons/ff0000/256/png/edit-property-icon-14-256.png',
    onClickEditKeyframe,
  );
  const deleteKeyframeButton = new CanvasButton(
    new Vector2D(160, 10),
    20,
    20,
    'https://cdn-icons-png.flaticon.com/512/3687/3687412.png',
  ...
  function isIntersectCircularBtn(centerPosition, radius, mouseX, mouseY) {
    let distanceFromCenter = Math.sqrt(
      (centerPosition.x - mouseX) ^ (2 + (centerPosition.y - mouseY)) ^ 2,
    );
  ...
  function renderKeyframeHorizontalLines(canvasContext) {
    ...
    for (var i = 0; i < numLines; i++) {
      ...
      canvasContext.fillRect(
        horizontalLineStartOffsetX,
        horizontalLineStartOffsetY + i * horizontalLineColumnAxisInterval,
  ...
  function renderKeyframeVerticalLine(canvasContext) {
    ...
    let progressRate =
      1.0 - (numberCounts - masterManager.sceneTimer.getPlayTime()) / parseFloat(numberCounts);
    ...
    canvasContext.fillRect(
      KVline.startXoffset + vlineXOffset,
      KVline.startYoffset,
      KVline.xLength,
      KVline.yLength,
    );
  ...
  function renderTimeNumberings(context) {
    for (var i = 0; i <= numberCounts; i++) {
      renderText(
        ...
        context,
        20,
        numberingStartOffsetX + i * numberingIntervalOffsetX,
        30,
        1,
  ...
  function renderKeyframes(context) {
    ...
    let keyframes = masterManager.sceneManager
      .getCurrentScene()
      .layerList[currentLayerIndex].getKeyframes();
    ...
    for (var i = 0; i < keyframes.length; i++) {
      ...
      context.beginPath();
      context.arc(
        horizontalLineStartOffsetX + keyframeXOffset,
        horizontalLineStartOffsetY,
        5,
        0,
        2 * Math.PI,
      );
    ...
    for (var i = 0; i < keyframes.length; i++) {
      ...
      if (i === 0) {
        for (var attr_idx = 0; attr_idx < 4; attr_idx++) {
          ...
          context.arc(
            horizontalLineStartOffsetX + keyframeXOffset,
            horizontalLineStartOffsetY + horizontalLineColumnAxisInterval * (attr_idx + 1),
            5,
            0,
            2 * Math.PI,
          );
        ...
        for (var attr_idx = 0; attr_idx < 4; attr_idx++) {
          ...
          switch (attr_idx) {
            case 0:
              if (
                Math.abs(prevKeyframe.position.x - currentKeyframe.position.x) < 0.00001 &&
                Math.abs(prevKeyframe.position.y - currentKeyframe.position.x) < 0.00001
              ) {
                isValueEqual = true;
              } else {
                isValueEqual = false;
            ...
            case 1:
              if (
                Math.abs(prevKeyframe.scale.x - currentKeyframe.scale.x) < 0.00001 &&
                Math.abs(prevKeyframe.scale.y - currentKeyframe.scale.x) < 0.00001
              ) {
                isValueEqual = true;
              } else {
                isValueEqual = false;
            ...
            case 3:
              if (
                Math.abs(prevKeyframe.image_fade_alpha - currentKeyframe.image_fade_alpha) < 0.00001
              ) {
                isValueEqual = true;
              } else {
                isValueEqual = false;
          ...
          if (isValueEqual) {
            //prevFrame의 attribute 값과 같다면, 그레이로 출력한다.
            context.fillStyle = 'gray';
          } //prevFrame의 attribute 값과 다르다면, 초록색으로 출력한다.
          else {
  ...
  function renderPlayButtons(context) {
    UIButtons.forEach((btn) => {
      if (btn.isLoaded) {
        ...
        if (btn.isHover) context.globalAlpha = 0.6;
  ...
  function bindMouseEvents() {
    ...
    canvasRef.current.addEventListener('mousedown', (e) => {
      ...
      for (var i = 0; i < UIButtons.length; i++) {
        let btn = UIButtons[i];
        let isClicked = isIntersectSquareRange(
          btn.position.x,
          btn.position.y,
          btn.xLength,
          btn.yLength,
      ...
      let keyframes = masterManager.sceneManager
        .getCurrentScene()
        .layerList[masterManager.sceneManager.currentLayerIndex].getKeyframes();
      ...
      for (var i = 0; i < keyframes.length; i++) {
        ...
        if (
          isIntersectSquareRange(
            horizontalLineStartOffsetX + keyframeXOffset - 5.0,
            horizontalLineStartOffsetY - 5.0,
            10,
            10,
            mouseX,
            mouseY,
  ...
  return (
    <>
      <canvas
        ...
        width={canvasWidth}
        height={canvasHeight}
        className={styles.keyframe_editor_canvas}
      ></canvas>
        ...
        <Modal>
          ...
          <div>
            <button
              onClick={() => {
                var currentLayer =
                  masterManager.sceneManager.getCurrentScene().layerList[
                    masterManager.sceneManager.currentLayerIndex
                  ];
                ...
                setKeyframeDeleteModalOpen(false);
              }}
            >
              확인
            </button>
